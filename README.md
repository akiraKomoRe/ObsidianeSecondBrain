...
期末評価の三区分のうち「部門定量項目」は、これまで名前に反して部門と無関係でした。
本人が期首に自由記述で立てるだけで、部長が定めた部門目標とのつながりがどこにもなく、
`profiles.department` はただの文字列で、部署という実体もその責任者も存在しませんでした。

いまは次の流れになっています。

1. 管理者が**部署マスタ**（`/admin/departments`）で部署・上位部署・部長を設定する
2. 部長が**部門目標**（`/team/department-goals`）で期ごとの目標と達成基準を定める
3. 本人が期首に、自分の部門定量項目を**どの部門目標に向けたものか**選ぶ
4. 上長の採点画面に、その紐付けがそのまま表示される

部署は入れ子にできます（工事第一課 は 工事本部 の下）。課の社員は自部署に加えて
**上位部署の目標も**選べ、部長は**配下の課の目標も**定められます。課長が上位部署の目標を
書き換えることはできません。

**配点は変えていません。** 人事評価規程 第5条の三区分・100点満点は不変で、部門目標は
「その個人目標が何に向けたものか」という文脈情報にとどめています（`score.ts` は無変更）。
紐付けは任意で、未選択でも保存できます — 部門目標がまだ立っていない期や、
どの部門目標にも直接は結びつかない個人目標があるためです。

締めた期の部門目標は誰も変更できません。確定した期末評価の根拠があとから書き換わると、
規程 第12条の不服申立てで参照するものが消えてしまうためです。

## 稼働日・休暇の扱い

「今週の日報 n/m件」の分母は、カレンダー上の平日ではなく**その人が実際に働く日**です。
会社休日（`company_holidays`）とその人の休暇（`personal_leaves`）を引いた日数が m になり、
休みの日は日報画面に「本日は◯◯です。提出は不要です」と出て、ホームの催促も出ません。
AI評価器にも稼働日数を渡しているので、有給を取った週が「記録が途切れている」とは書かれません。

判定は `src/lib/attendance/working-days.ts` の `WorkingDaySource` に閉じています。
呼び出し側（ホーム・チーム画面・日報画面・AI評価器）はこのインターフェースだけを見ているので、
勤怠の出どころが変わってもそこは書き換えません。

### ジョブカン勤怠との連携（未実装）

`JobcanWorkingDaySource` はインターフェースだけ用意してあり、中身は空です。
**ジョブカン勤怠APIの実仕様をこちらで確認できていない**ため（開発環境からヘルプセンターに
到達できず、記憶で書くと誤りになる）、実装前に以下の確認が必要です。

1. 契約プラン（APIが使えるプランかどうか）
2. APIトークンの発行方法と、取得できるエンドポイント
3. 勤怠区分（出勤／有休／欠勤／振替休日…）のコード体系
4. ジョブカン側の社員IDと `profiles.id` の対応をどう持つか

これが揃うまでは、既定のカレンダー方式（会社休日は管理画面から登録、
休暇は `personal_leaves` に登録）で運用できます。連携後は `personal_leaves` を
ジョブカンからの同期先にし、`source` 列で手入力ぶんと区別します。
切り替えは環境変数 `ATTENDANCE_SOURCE=jobcan`。

## AIに投げるプロンプトの管理

プロンプト本文・ツール定義・版番号はすべて `src/lib/evaluation/prompt.ts` の1ファイルに
まとまっています。**本文を変えたら `WEEKLY_PROMPT_VERSION` を上げる**のがルールです。

生成された評価の行には、モデル名（`model_version`）と併せてこの版番号（`prompt_version`）が
記録され、AI週次評価の詳細画面に小さく表示されます。人事評価規程 第12条で本人が評価に
不服を申し立てたとき、審査するのは「どういう基準で出た点なのか」であって、モデル名だけでは
それが特定できないためです（同じモデルでも基準を書き換えれば点は変わる）。

本文自体はデータベースに置いていません。プロンプトはコードと同じ速度で変わるので、
差分レビューが効くgit履歴を唯一の正とし、行に残すのは版番号だけにしています。

なお `temperature` は設定していません。設定できません —— 現行モデル（`claude-sonnet-5` /
`claude-opus-5` など）では `temperature`・`top_p`・`top_k` が廃止されており、送ると400で落ちます。
「同じ日報なら毎回同じ点数」は温度ではなく、**生成結果をそのまま保存して以後はその保存値を
読む**ことで担保しています。再生成しない限り評点は動きません。
...
| `src/lib/local/policy.test.ts` | **公開前に本人が上長評価を見られないこと**、部門目標を書けるのが部長だけであること、休暇が同僚に見えないこと |
| `src/lib/evaluation/local-generate.test.ts` | ローカル評価器が全項目満点にならず、記述内容を反映すること。**有給の週を「記録が途切れている」と書かないこと** |
| `src/lib/attendance/working-days.test.ts` | 会社休日と本人の有給が稼働日から外れ、他人の有給では減らないこと |
...
- ジョブカン勤怠との連携本体（インターフェースのみ用意済み。上記「ジョブカン勤怠との連携」参照）
- 部門目標の達成度そのものの評価（現在は個人目標の紐付け先として持つだけで、
  部門目標に点は付かない。規程に部門評価の区分が無いため）
